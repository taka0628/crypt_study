...
{
#define LEN (93)
    int len = LEN; /* is not multiples of 16. */
    ...
    char in[] = {"hogehogetestertstenjkshdfbnwseuibfobgjkbsdfikgbisuhfbdsigbadsiulgsbdfsdhbfidflgvbsdjbfgisufb"};
    printf("in: %s\nsize: %ld\n", in, sizeof(in));
    static uint8_t out[LEN];
    static uint8_t out2[LEN];
    

    encrypt(out, in, key, iv, 93);
    decrypt(out2, out, key, iv, 95);
    // Encrypt(key, in, sizeof(in), iv, out, sizeof(out));
    // Decrypt(key, out, sizeof(out), iv, out2, sizeof(out2));
    printf("en: %s\n", out);
    printf("de: %s\n", out2);
